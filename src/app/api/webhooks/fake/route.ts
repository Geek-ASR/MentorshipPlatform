 ...
 * never on the production site (`APP_ENV`, not `NODE_ENV`: every `next start` build — e2e, a
 * sandbox — runs with NODE_ENV=production), matching the dev checkout's own gate; config
 * validation also refuses the fake provider when APP_ENV=production. Reads raw bytes before any JSON
